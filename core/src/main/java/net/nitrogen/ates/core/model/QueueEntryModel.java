...
public class QueueEntryModel extends Model<QueueEntryModel> {
	...
	public List<QueueEntryModel> findAllEntriesAsModelList() {
		return find(String.format("SELECT `%s`,`%s`,`%s`,`%s`,`%s`,`%s`,`%s`,`%s`,`%s`,`%s`,`%s`,`%s` FROM `%s` ORDER BY `%s` DESC", Fields.ID, Fields.STATUS,
				Fields.NAME, Fields.SLAVE_NAME, Fields.INDEX, Fields.START_TIME, Fields.END_TIME, Fields.EXECUTION_ID, Fields.PROJECT_ID, Fields.ENV,
				Fields.JVM_OPTIONS, Fields.PARAMS, TABLE, Fields.ID));
	}

	public List<QueueEntryModel> findEntriesAsModelList(long executionId) {
		String sql = String.format("SELECT `%s`,`%s`,`%s`,`%s`,`%s`,`%s`,`%s`,`%s`,`%s`,`%s`,`%s`,`%s` FROM `%s` WHERE `%s`=? ORDER BY `%s` DESC", Fields.ID,
				Fields.STATUS, Fields.NAME, Fields.SLAVE_NAME, Fields.INDEX, Fields.START_TIME, Fields.END_TIME, Fields.EXECUTION_ID, Fields.PROJECT_ID,
				Fields.ENV, Fields.JVM_OPTIONS, Fields.PARAMS, TABLE, Fields.EXECUTION_ID, Fields.ID);
	...
	public void insertEntries(List<QueueEntryModel> entries) {
		...
		if (foundEntries.size() > 0) {
			final String sql = String.format("INSERT INTO `%s`(`%s`,`%s`,`%s`,`%s`,`%s`,`%s`,`%s`,`%s`) VALUES(?,?,?,?,?,?,?,?)", TABLE, Fields.STATUS,
					Fields.NAME, Fields.SLAVE_NAME, Fields.EXECUTION_ID, Fields.PROJECT_ID, Fields.ENV, Fields.JVM_OPTIONS, Fields.PARAMS);
	...
	private static Connection getDBConnection() {
		...
		try {
			dbConnection = DriverManager.getConnection("jdbc:mysql://127.0.0.1:3306/nitrogenates?characterEncoding=utf8&zeroDateTimeBehavior=convertToNull",
					"nitrogenadmin", "@ctive123");
			return dbConnection;
